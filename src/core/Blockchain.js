...
export class Blockchain {
  ...
  async createNewTransactions(transactions, cpuCostAddress) {
    ...
    if (cpuCostAddress) {
      ...
      const feeTransactions = transactions.map((tx) => new Transaction(cpuCost / transactions.length, tx.to, cpuCostAddress));
  ...
  coinsInEco() {
    return this.chain.reduce((sum, block) => sum + block.transactions.reduce((blockSum, tx) => blockSum + (tx.from === '00' ? tx.amount : 0), 0), 0);
  }

  coinsInWallet(address = this.getCoinOwnerAddress()) {
    return this.chain.reduce((sum, block) => sum + block.transactions.reduce((blockSum, tx) => blockSum + (tx.to === address ? tx.amount : 0) - (tx.from === address ? tx.amount : 0), 0), 0);
  }
}
